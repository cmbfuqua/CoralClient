...
from utility_functions import admin_required

from models import User
from billingmodels import Bill, MaintenanceVisit, BillLineItem, ChemicalRanges
from billingforms import AddLineItemForm, MaintenanceVisitForm
from DB import db
...
def generate_invoices_all():
    ...
    def process_customer(app_instance, data):
        ...
        with app_instance.app_context():  # Push the actual passed application context
...
def process_bill_status(bill_id):
    ...
    return redirect(url_for('billing.create_maintenance_visit'))  # Redirect to maintenance visit creation
...
@billing_bp.route('/generate_monthly_bills', methods=['POST'])
@login_required
